...
const tableColumns = computed<TableColumn[]>(() => [
  ...
  { key: "student_id", label: t("Student ID"), sortable: true },
  {
    ...
    sortable: true,
    render: (r) => renderStudentName(r as Student),
  },
  {
    key: "roll_no",
    label: t("Roll No"),
    sortable: true,
    align: "center",
    render: (r) => String((r as Student).roll_no ?? "—"),
  },
]);
...
function filterRoster() {
  ...
  filteredStudents.value = students.value.filter(
    (s) =>
      Number(s.academic_year_id) === Number(activeYearId.value) &&
      s.class_name === activeClass.value &&
      s.is_active,
  );
}
...
watch(
  ...
  (newList) => {
    ...
    for (const s of newList) {
      if (s.id) {
        newMap[s.id] =
          selectedStudentIds.value[s.id] !== undefined ? selectedStudentIds.value[s.id] : true;
      }
    }
...
function printCard() {
  ...
  toast.success(
    t("Opening system print dialog for {count} cards...", { count: checkedStudents.value.length }),
  );
  setTimeout(() => {
    window.print();
  }, 500);
    ...
    <header class="ipf-header">
      <div class="ipf-header__titles">
        ...
        <p>
          {{
            t(
              "Generate, preview, and print beautiful, customized sessional student ID cards on-the-fly. Prints 8 cards per A4 page.",
            )
          }}
        </p>
      </div>
      <div class="ipf-header__actions">
        <button
          ...
          @click="printCard"
        >
          <i class="fa-duotone fa-print" /> {{ t("Print Selected Cards") }} ({{
    ...
    <div class="id-cards-grid-layout">
      ...
      <div class="roster-table-block ipf-section">
        ...
        <DataTable
          ...
          :empty-text="
            t('No active student records found. Select a different class or academic year to load.')
          "
        >
          ...
          <template #select="{ row }">
            <input
              ...
              @change="
                selectedStudentIds[(row as Student).id!] = (
          ...
          <template #actions="{ row }">
            <button
              type="button"
              class="btn btn--primary"
              @click="triggerRowPreview(row as Student)"
            >
      ...
      <div class="id-card-preview-panel">
        <div class="id-card-preview-top">
          ...
          <div v-if="previewTarget" class="live-mockup-wrapper animate-fade-in">
            <div class="std-id-card-preview" :class="[cardOrientation, cardTemplate]">
              ...
              <div v-if="cardOrientation === 'orientation-landscape'" class="card-middle-content">
                ...
                <div class="std-id-card-info">
                  ...
                  <p>
                    {{ t("ID:") }} <strong>{{ previewTarget.student_id }}</strong>
                  </p>
                  <p>{{ t("Class:") }} {{ previewTarget.class_name }}</p>
                  <p>
                    {{ t("Sec:") }} {{ previewTarget.section_name }} · {{ t("Roll:") }}
                    {{ previewTarget.roll_no }}
                  </p>
                  <p v-if="previewTarget.blood_group">
                    {{ t("Blood:") }} {{ previewTarget.blood_group }}
                  </p>
                </div>
              </div>
              ...
              <template v-else>
                ...
                <div class="std-id-card-info">
                  <h4 class="card-student-name">{{ previewTarget.candidate_name }}</h4>
                  <p>
                    {{ t("Student ID:") }} <strong>{{ previewTarget.student_id }}</strong>
                  </p>
                  <p>
                    {{ t("Class:") }} {{ previewTarget.class_name }} · {{ t("Section:") }}
                    {{ previewTarget.section_name }}
                  </p>
                  <p>{{ t("Roll No:") }} {{ previewTarget.roll_no }}</p>
                  <p v-if="previewTarget.blood_group">
                    {{ t("Blood Group:") }} {{ previewTarget.blood_group }}
                  </p>
                </div>
              </template>
            ...
            <div class="validity-indicators">
              <span class="status-badge status-badge--success badge-sm"> ✓ {{ t("Active") }} </span>
              <span
              ...
              <span
                class="status-badge badge-sm"
                :class="
                  previewTarget.government_uid && previewTarget.government_uid.length === 17
          ...
          <div v-else class="empty-preview-mockup">
            ...
            <p class="text-muted empty-text">
              {{ t("Select a student from the register roster list to focus preview card.") }}
            </p>
          </div>
        </div>
        ...
        <div v-if="previewTarget" class="panel-print-actions">
          <button type="button" class="btn btn--primary w-full" @click="printCard">
            <i class="fa-duotone fa-print" /> {{ t("Print Roster Cards") }} ({{
              checkedStudents.length
            }})
          </button>
        </div>
      ...
      >
        ...
        <div v-if="cardOrientation === 'orientation-landscape'" class="card-middle-content">
          ...
          <div class="std-id-card-info">
            ...
            <p>
              {{ t("ID:") }} <strong>{{ s.student_id }}</strong>
            </p>
        ...
        <template v-else>
          ...
          <div class="std-id-card-info">
            ...
            <p>
              {{ t("Student ID:") }} <strong>{{ s.student_id }}</strong>
            </p>
