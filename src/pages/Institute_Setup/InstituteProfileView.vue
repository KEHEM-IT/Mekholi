<!-- Institute Setup > Profile & EIIN -->
<script setup lang="ts">
import { computed, reactive, ref } from 'vue'
import { useAppPreferences } from '@/composables/useAppPreferences'
import profileJson from '@/assets/school/profile_display.json'
import type { GeneralInfo } from '@/assets/school/converter'
...
<template>
  <section class="ipf">
    <header class="ipf-header">
      <h1>{{ isBn ? 'প্রোফাইল ও ইআইআইএন' : 'Profile & EIIN' }}</h1>
      <p>
        {{
          isBn
          ...
          <div class="ipf-section__body">
            <div class="ipf-grid">
              <div class="form-field">
                ...
                <input v-model="profile.institute_name_bn" type="text" :placeholder="isBn ? 'নাম লিখুন' : 'Enter name'" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'প্রতিষ্ঠানের নাম (ইংরেজি)' : 'Institute Name (English)' }}</label>
                <input v-model="profile.institute_name_en" type="text" :placeholder="isBn ? 'নাম লিখুন' : 'Enter name'" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'প্রতিষ্ঠাতা' : 'Founder' }}</label>
                <input v-model="profile.founder_name" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'প্রতিষ্ঠান প্রধানের নাম' : 'Head of Institute' }}</label>
                <input v-model="profile.head_of_institute_name" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'প্রতিষ্ঠার তারিখ' : 'Establishment Date' }}</label>
                <input v-model="profile.establishment_date" type="text" placeholder="DD/MM/YYYY" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'সংসদীয় আসন' : 'Parliamentary Constituency' }}</label>
                <input v-model="profile.parliamentary_constituency" type="text" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="ipf-section__body">
            <div class="ipf-grid ipf-grid--three">
              <div class="form-field">
                ...
                <input v-model="profile.address.village_road_holding_no" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'মৌজা' : 'Mouza' }}</label>
                <input v-model="profile.address.mouza_name" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'দাগ নম্বর' : 'Plot / Dag No.' }}</label>
                <input v-model.number="profile.address.plot_dag_number" type="number" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ডাকঘর' : 'Post Office' }}</label>
                <input v-model="profile.address.post_office" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'পোস্ট কোড' : 'Post Code' }}</label>
                <input v-model.number="profile.address.post_code" type="number" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ইউনিয়ন' : 'Union' }}</label>
                <input v-model="profile.address.union" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'উপজেলা/থানা' : 'Upazila / Thana' }}</label>
                <input v-model="profile.address.upazila_thana" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'জেলা' : 'District' }}</label>
                <input v-model="profile.address.district" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'বিভাগ / অঞ্চল' : 'Division / Region' }}</label>
                <input v-model="profile.address.division_region" type="text" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="ipf-section__body">
            <div class="ipf-grid">
              <div class="form-field">
                ...
                <input v-model.number="profile.contact.institute_phone" type="number" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'প্রধান মোবাইল' : 'Head Mobile' }}</label>
                <input v-model.number="profile.contact.head_of_institute_mobile" type="number" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ফ্যাক্স' : 'Fax' }}</label>
                <input v-model="profile.contact.fax" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ইমেইল' : 'Email' }}</label>
                <input v-model="profile.contact.institute_email" type="email" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ওয়েবসাইট' : 'Website' }}</label>
                <input v-model="profile.contact.website" type="text" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="ipf-section__body">
            <div class="ipf-grid ipf-grid--three">
              <div class="form-field">
                ...
                <input v-model="profile.classification.institute_type" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'সংযুক্ত কারিগরি শাখা' : 'Attached Tech. Branch' }}</label>
                <input v-model="profile.classification.attached_technical_branch_type" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'গ্রুপ' : 'Group' }}</label>
                <input v-model="profile.classification.group" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'কাদের জন্য' : 'Student Type' }}</label>
                <input v-model="profile.classification.student_type" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'শিফট সংখ্যা' : 'Shift Count' }}</label>
                <input v-model="profile.classification.shift_count" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ইংরেজি ভার্সন' : 'English Version' }}</label>
                <input v-model="profile.classification.has_english_version" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ব্যবস্থাপনা' : 'Management' }}</label>
                <input v-model="profile.classification.management" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'স্বীকৃতির অবস্থা' : 'Recognition Status' }}</label>
                <input v-model="profile.classification.recognition_status" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'স্বীকৃতির স্তর' : 'Recognized Level' }}</label>
                <input v-model="profile.classification.recognized_level" type="text" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="ipf-section">
          ...
          <div class="ipf-section__body">
            <div class="ipf-grid ipf-grid--three">
              <div class="form-field">
                <label>EIIN</label>
                <input v-model="profile.identifiers.eiin" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'জিইও কোড' : 'Geo Code' }}</label>
                <input v-model="profile.identifiers.geo_code" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'বোর্ড কোড' : 'Board Code' }}</label>
                <input v-model="profile.identifiers.board_institute_code" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'কারিগরি বোর্ড কোড' : 'Technical Board Code' }}</label>
                <input v-model="profile.identifiers.technical_board_code" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'এমপিও কোড' : 'MPO Code' }}</label>
                <input v-model="profile.identifiers.mpo_code" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'কারিগরি এমপিও কোড' : 'Tech. Branch MPO Code' }}</label>
                <input v-model="profile.identifiers.technical_branch_mpo_code" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'উপবৃত্তি কোড' : 'Stipend Code' }}</label>
                <input v-model="profile.identifiers.stipend_code" type="text" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="ipf-section__body">
            <div class="ipf-grid">
              <div class="form-field">
                ...
                <input v-model="profile.mpo_status.is_mpo_enrolled" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'কারিগরি শাখা এমপিওভুক্ত?' : 'Technical Branch MPO?' }}</label>
                <input v-model="profile.mpo_status.technical_branch_mpo_status" type="text" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="ipf-section__body">
            <div class="ipf-grid ipf-grid--three">
              <div class="form-field">
                ...
                <input v-model="profile.location_details.nationalization_date" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'নিকটবর্তী প্রশাসনিক ইউনিট' : 'Nearest Admin Unit' }}</label>
                <input v-model="profile.location_details.nearest_admin_unit" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'দূরত্ব (কিমি)' : 'Distance (km)' }}</label>
                <input v-model.number="profile.location_details.nearest_admin_unit_distance_km" type="number" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'এলাকার ধরণ' : 'Area Type' }}</label>
                <input v-model="profile.location_details.area_type" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ভৌগোলিক অবস্থান' : 'Geographic Location' }}</label>
                <input v-model="profile.location_details.geographic_location" type="text" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ছিটমহল?' : 'Enclave?' }}</label>
                <input v-model="profile.location_details.is_enclave" type="text" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="ipf-section__body">
            <div class="ipf-grid ipf-grid--three">
              <div class="form-field">
                ...
                <input v-model.number="profile.income_total" type="number" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'মোট ব্যয়' : 'Total Expense' }}</label>
                <input v-model.number="profile.expense_total" type="number" />
              </div>
              <div class="form-field">
                <label>{{ isBn ? 'ছাত্র বেতন ও সেশনচার্জ' : 'Student Fee Amount' }}</label>
                <input v-model.number="profile.student_fee_amount" type="number" />
              </div>
            </div>
          </div>
        </div>
