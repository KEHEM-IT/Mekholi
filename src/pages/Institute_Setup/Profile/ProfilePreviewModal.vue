...
const divisionName = computed(() =>
  geoName(BD_GEO_DIVISIONS as unknown as { id: string; LookupText: string }[], f.value.division_id),
);
const districtName = computed(() =>
  geoName(BD_GEO_DISTRICTS as unknown as { id: string; LookupText: string }[], f.value.district_id),
);
const upazilaName = computed(() =>
  geoName(BD_GEO_UPAZILAS as unknown as { id: string; LookupText: string }[], f.value.upazila_id),
);
const unionName = computed(() =>
  geoName(BD_GEO_UNIONS as unknown as { id: string; LookupText: string }[], f.value.union_id),
);

const facilities = computed<Record<string, boolean>>(
  () => (f.value.facilities as Record<string, boolean>) ?? {},
);
const classifications = computed<Array<Record<string, unknown>>>(() =>
  Array.isArray(f.value.classifications)
    ? (f.value.classifications as Array<Record<string, unknown>>)
    : [],
);
...
const committee = computed<Record<string, unknown>[]>(
  () => (f.value.committee_members as Record<string, unknown>[]) ?? [],
);
...
const staffTotal = computed(() => {
  ...
  return total > 0
    ? total
    : (Number(f.value.staff_male) || 0) + (Number(f.value.staff_female) || 0);
});
    ...
    <div class="ipfp-body">
      <div class="ipfp-section" v-if="identityFields.some((x) => show(f[x.key]) !== '—')">
        <h4 class="ipfp-section__title">
          <i class="fa-duotone fa-id-card" /> {{ L("Identity", "পরিচয়") }}
        </h4>
        <div class="ipfp-grid">
      ...
      <div class="ipfp-section" v-if="addressFields.some((x) => show(f[x.key]) !== '—')">
        <h4 class="ipfp-section__title">
          <i class="fa-duotone fa-location-dot" /> {{ L("Address", "ঠিকানা") }}
        </h4>
        <div class="ipfp-grid">
      ...
      <div class="ipfp-section" v-if="contactFields.some((x) => show(f[x.key]) !== '—')">
        <h4 class="ipfp-section__title">
          <i class="fa-duotone fa-phone" /> {{ L("Contact", "যোগাযোগ") }}
        </h4>
        <div class="ipfp-grid">
      ...
      <div class="ipfp-section" v-if="classFields.some((x) => show(f[x.key]) !== '—')">
        <h4 class="ipfp-section__title">
          <i class="fa-duotone fa-graduation-cap" /> {{ L("Classification", "শ্রেণিবিভাগ") }}
        </h4>
        <div class="ipfp-grid">
      ...
      <div class="ipfp-section" v-if="idFields.some((x) => show(f[x.key]) !== '—')">
        <h4 class="ipfp-section__title">
          <i class="fa-duotone fa-fingerprint" /> {{ L("Identifiers", "শনাক্তকরণ") }}
        </h4>
        <div class="ipfp-grid">
      ...
      <div
        class="ipfp-section"
        v-if="staffFields.some((x) => show(f[x.key]) !== '—') || staffTotal > 0"
      >
        <h4 class="ipfp-section__title">
          <i class="fa-duotone fa-users" /> {{ L("Staff", "কর্মচারী") }}
        </h4>
        <div class="ipfp-grid">
      ...
      <div class="ipfp-section" v-if="bankFields.some((x) => show(f[x.key]) !== '—')">
        <h4 class="ipfp-section__title">
          <i class="fa-duotone fa-piggy-bank" /> {{ L("Bank Account", "ব্যাংক হিসাব") }}
        </h4>
        <div class="ipfp-grid">
      ...
      <div class="ipfp-section">
        <h4 class="ipfp-section__title">
          <i class="fa-duotone fa-grid-2" /> {{ L("Facilities", "সুবিধাদি") }}
        </h4>
        <div class="ipfp-facilities">
          ...
          >
            ...
            <i
              class="fa-solid ipfp-facility__mark"
              :class="facilities[key] ? 'fa-check' : 'fa-xmark'"
            />
          </span>
        </div>
      </div>
      ...
      <div class="ipfp-section" v-if="committee.length">
        ...
        <div class="ipfp-committee">
          <div v-for="(m, i) in committee" :key="i" class="ipfp-member">
            <div class="ipfp-member__avatar">
              {{ (String(m.member_name ?? "") || "?").trim().charAt(0).toUpperCase() }}
            </div>
